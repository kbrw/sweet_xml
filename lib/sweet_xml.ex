...
defmodule SweetXml do
  ...
  Nested Mapping:
      ...
      %{a_in_li: [~c'Two'], message: ~c'Message'}
  ...
  `~x`, you can do the following
      ...
      iex> doc = "<h1><a>Some linked title</a></h1>"
      iex> doc |> SweetXml.xpath(%SweetXpath{path: ~c'//a/text()', is_value: true, cast_to: false, is_list: false, is_keyword: false})
      ~c'Some linked title'
  ...
  boolean fields:
      ...
      iex> SweetXml.sigil_x("//some/path", ~c'e')
      %SweetXpath{path: ~c'//some/path', is_value: false, cast_to: false, is_list: false, is_keyword: false}
  ...
  * `:dtd` to prevent DTD parsing or fetching, with the following possibilities:
  ...
  def stream_tags(doc, tags, options \\ []) do
    ...
    {discard_tags, xmerl_options} =
      case :proplists.lookup(:discard, options) do
  ...
  def stream(doc, options_callback) do
    Stream.resource(
      fn ->
        ...
        pid =
          spawn_link(fn -> :xmerl_scan.string(~c"", opts ++ continuation_opts(doc, waiter)) end)

        {ref, pid, Process.monitor(pid), do_after}
      end,
      fn {ref, pid, monref, do_after} = acc ->
        receive do
  ...
  def stream!(doc, options_callback) do
    Stream.resource(
      fn ->
        ...
        {pid, monref} =
          spawn_monitor(fn ->
            :xmerl_scan.string(~c"", opts ++ continuation_opts(doc, waiter))
          end)
  ...
  For more details, see `parse/2`.
  """
  @spec xmap(parent :: doc | xmlElement, mapping :: specs, options :: boolean | map) ::
          map | keyword
