defmodule SweetXml.Options do
  ...
  def handle_dtd(:none, exception_module) do
    fn ets ->
      handle_dtd(:internal_only, exception_module).(ets) ++
        handle_dtd([only: []], exception_module).(ets)
    end
  end
    ...
    case exception_module do
      SweetXml.DTDError ->
        fn _ ->
          [
            fetch_fun: fn _, _ ->
              raise SweetXml.DTDError, message: "no external entity allowed"
            end
          ]
        end
  ...
  def set_up(opts, dtd_exception_module) do
    ...
    ret =
      {_opts, _do_after} =
      case :proplists.split(opts, [:rules]) do
        {[[]], opts} ->
          ets = :ets.new(nil, [:public])

          opts =
            opts ++
              SweetXml.Options.handle_dtd(dtd_arg, dtd_exception_module).(ets) ++ [rules: ets]
